...
class TestGates(unittest.TestCase):
    ...
    def test_basic_gates(self):
        ...
        for gate in [X, Y, Z, H]:
            ...
            self.assertIn(gate.as_qobj_openQASM()[
                          'name'], ['x', 'y', 'z', 'h'])
            self.assertEqual(gate.as_qobj_openQASM()['params'], [])
            self.assertEqual(gate.as_qobj_openQASM()['qubits'], [3])
            g_copy = copy(gate)
